...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(redirectTo, { replace: true, state: { message: 'Welcome back!' } });
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#030303] flex flex-col justify-center items-center px-4 relative overflow-hidden select-none">
      ...
      <div className="w-full max-w-sm z-10 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLoading ? <Loader2 className="animate-spin" size={16} /> : <>Sign In <ArrowRight size={15} /></>}
          </button>
        </form>

        <div className="my-6 flex items-center gap-4 text-slate-700">
          <div className="h-px flex-1 bg-white/5" />
          <span className="text-[8px] font-mono uppercase tracking-widest shrink-0 select-none">Or login with</span>
          <div className="h-px flex-1 bg-white/5" />
        </div>
